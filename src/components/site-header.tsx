...
export function SiteHeader() {
  ...
  useEffect(() => {
    ...
    void loadConversationSummaries(user.id).then(async (rows) => {
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="messages-menu">
                <button type="button" className="messages-trigger" aria-label="Open messages" aria-expanded={messagesOpen} onClick={() => setMessagesOpen((open) => !open)}>⌁{conversations.reduce((total, row) => total + row.unreadCount, 0) > 0 && <b>{conversations.reduce((total, row) => total + row.unreadCount, 0)}</b>}</button>
